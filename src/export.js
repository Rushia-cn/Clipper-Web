function formatName(name) {
  const formatted = Object.entries(name)
    .map(e => {
      if (!e[1]) return
      return `${e[0]}:"${e[1]}"`
    })
...
function genBat(vm, item) {
  ...
  return `${vm.videoURL} ${vm.$t2s(item.from)} ${vm.$t2s(item.to)} ${
    item.cat ? item.cat : ''
...
function exportAsYtDl(items, vm) {
  ...
  function genCmd(item) {
    ...
    return `ffmpeg -y -i "${vid}.mp3" -ss ${from} ${
      to ? `-to ${to}` : ''
    } -af loudnorm=I=-23:TP=-2:LRA=11 ${vid}-${from}${to ? `-${to}` : ''}.mp3`
  }
...
export default [
  ...
  },
  {
    ...
    method: () => {
      window.open(
        'https://github.com/Rushia-cn/Rushia-button/issues/new?assignees=GeorgeMiao219&labels=&template=new-clip-request.md&title=%5BClip%5D'
      )
    }
  },
  {
